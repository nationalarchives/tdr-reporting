...
def slack(event, environment, csv_file_path):
    # Initialize Slack client to match test base URL
    client = WebClient(token=decode("SLACK_BOT_TOKEN"), timeout=180, base_url="https://www.slack.com/api/")
    # Proceed only if userName provided and non-empty
    user_names = event.get("userName") or []
    if not user_names:
        return
    # Lookup each user by email, raise on invalid auth
    user_ids = []
